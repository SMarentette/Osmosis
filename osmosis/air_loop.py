...
class MermaidDiagram:
    ...
    def _repr_html_(self) -> str:
        ...
        if self.zoomable:
            return self._repr_zoomable_html_(url, alt, width)
        return f"""
<img
  src="{url}"
  alt="{alt}"
...
>
  ...
  {inputs}
  <div class="toolbar"><span>Zoom</span>{labels}</div>
  ...
  <script>
    (() => {{
      ...
      const image = root.querySelector(".viewport img");
      const hScrolls = [...root.querySelectorAll(".h-scroll")];
      const hSpacers = [...root.querySelectorAll(".h-spacer")];
      let syncing = false;

      const setSpacerSize = () => {{
        const width = image.scrollWidth || image.getBoundingClientRect().width;
      ...
      if (image.complete) {{
        ...
        image.addEventListener("load", setSpacerSize);
      }}
      if (window.ResizeObserver) {{
        new ResizeObserver(setSpacerSize).observe(image);
      }}
    }})();
  </script>
</div>
...
class AirLoopHVAC(OsmObject):
    ...
    def _build_mermaid(self, show_names: bool = True) -> str:
        ...
        lines = [
            ...
            "    linkStyle default stroke:#1f2933,stroke-width:1.35px;",
        ]
        ...
        for comp in raw.supplyComponents():
